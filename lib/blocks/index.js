...
const DIFFICULTY_AVG_BLOCKS = 100;
...
module.exports.getNextDifficultyByBlock = function(block) {
    ...
    let calcStartHeight = block.header.height - DIFFICULTY_AVG_BLOCKS;
    if(calcStartHeight < 2) {
        calcStartHeight = 2
    }
    let calcBlocksCount = block.header.height - calcStartHeight;

    let block1 = storage.getBlockByHeight(calcStartHeight);
    let blockTimeDiff = block.header.timestamp - block1.header.timestamp;
    let nowAvgTimePerBlock = Math.floor(blockTimeDiff/calcBlocksCount);
