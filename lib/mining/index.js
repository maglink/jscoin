...
class Miner {
    constructor() {
        this.coinbaseAddress = transactions.getAddressFromPubKey(config.data.wallet.pubKey);
        ...
        transactions.storage.onTransactionsUpdate(() => {
            if(!this.mineInterval) {
                return;
            }
            this.updateTrxs();
    ...
    updateTrxs() {
        ...
        _self.trxs.push(coinbase.getData());
        blockLengthSum += JSON.stringify(coinbase.getData()).length;

        let trxsFromMempool = transactions.storage.getTransactions();
        for(let trxHash in trxsFromMempool) {
            let trx = trxsFromMempool[trxHash];
